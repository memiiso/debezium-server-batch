...
public class TestSparkIcebergConsumer extends BaseSparkTest {
  ...
  public void testDatatypes() throws Exception {
    ...
    sql =
        "INSERT INTO inventory.table_datatypes ("
            + "c_id, "
        ...
        .until(
            () -> {
              try {
                Dataset<Row> df =
                    spark.sql(
                        "select * from default.testc_inventory_table_datatypes where c_id = 2");
    ...
    Awaitility.await()
        ...
        .until(
            () -> {
              try {
                Dataset<Row> df =
                    spark.sql("select * from default.testc_inventory_table_datatypes");
  ...
  public void testUpdateDeleteDrop() throws Exception {
    ...
    //    TestDatabase.runSQL("ALTER TABLE inventory.customers ADD test_varchar_column
    // varchar(255);");
    ...
    TestDatabase.runSQL(
        "INSERT INTO inventory.customers VALUES " + "(default,'SallyUSer2','Thomas',null);");
    ...
    TestDatabase.runSQL(
        "INSERT INTO inventory.customers VALUES " + "(default,'User3','lastname_value3');");
    Awaitility.await()
        .atMost(Duration.ofSeconds(60))
        .until(
            () -> {
